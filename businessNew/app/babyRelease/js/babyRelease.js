...
require('!style!css!sass!../scss/babyRelease.scss');

var Vue = require('../../../public/lib/vue.js');
var text = require('../view/babyRelease.html');
var $state = require('../../../public/component/$state.js');
var tool = require('../../../public/component/toollib.js');

var babyReleaseService = require('./babyReleaseService.js');
...
module.exports = scope = {
    ...
    putaway: function () { //上架
        ...
        if (selectArray.length !== 0 && shopName !== '' && property && copyreaderStatus && descriptionStatus && shopClassify) {
            alert(JSON.stringify(merchandiseInfo));
    ...
    merchandiseInfoWay: function () {
        ...
        if (standardContent) {
            $.each(standardContent[0], function (index, item) {
                self.$scope.merchandiseInfo[0].merchandiseSkus.push({
                    attrSpec: [],
                    imgIndex: '',
                    linePrice: item.shitil,
                    onLinePrice: item.pingtai,
                    qty: item.kucun,
                    skuCode: item.shangpin,
                    skuStatus: 1
                });
                self.$scope.merchandiseInfo[0].merchandiseSkus.attrSpec.push({
